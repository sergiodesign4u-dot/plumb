...
## Stage 04 Wireframes, 2026-08-19

**Why the number card is the reference screen, and why flow 1 was assembled first.** The card is the workhorse: nine of the sixteen pages of the first round come out of its template, its compact form is the row of the registry and of the search results, and it is rendered live inside the product page, which node 2.1 records as three copies to walk when the canon changes. The alternative was the registry, which would have established the analyst's header and the listing a step earlier. It was refused for a reason this project keeps having to defend: **building the planet-setting screen for the persona the interface is deliberately not built around inverts the priority the IA spent a stage arguing.** Flow 1 followed from the choice, and it is also the flow that closes the job the product exists for.

**What the fan-out cost and what it bought.** Nine agents wrote 36 pages against one contract, and the most useful thing they returned was not markup. Six of them hit the same wall independently: node 0.2 defines three footer variants and the component knew one, so some shipped the reader's trust line under an analyst's settings form and some refused to render a footer at all and said so. **A component that knows one variant lies on two thirds of the product**, and six independent reports are what made that obvious rather than arguable.

**The consolidation is where the real defect lived.** Folding nine private stylesheets into one turned up six selectors defined twice with different bodies: the listing, its cells, the label, the banner, the disabled button and the breadcrumb. Left alone that arrives at stage 07 as three versions of one component and becomes visible two stages later as a fork in the tokens. The merge was made neutral by construction and proved by reading computed styles before and after rather than by assertion.

**What the critique gave, and it is an argument for radii rather than opinions.** Five instruments: Codex on the source, three auditors in a browser, a grep, and a measurement of all 52 pages. **Codex found nothing at 360 because it cannot see a rendered page, and the auditors found four defects there**, including a placeholder that never rendered at all because height and width were set on an inline span. A grep saw that rule and was satisfied. In the other direction, Codex and the grep independently agreed that no rule survives on two screens, which is what proves the consolidation held.

**Three findings were IA holes and were not repaired in the wireframe.** Search is specified twice, in node 0.1 as a persistent affordance in the bar and in node 4.1 as a block on the page, and rendering both put two search fields on one screen. The workspace tells an administrator to change seats on a page with no seat control and to change a role that no screen can change. And node 0.2 has three footer variants, none of which fits a gate. **The prototype carries a provisional treatment for the first and nothing for the other two**, because a wireframe that repairs an IA hole hides it.
