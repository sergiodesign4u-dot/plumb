...
**Refined at CJM, and this is the current version.** The full table with parents and success signals is the backlog in `research/docs/cjm-to-be.md`; three results come back here because they change what gets built. **The definition is visible on the card, the source opens on request**, since principle 3 makes the definition the main object and the first CJM draft had it collapsed. **Two capabilities were missing and are now in:** a definition snapshot taken at read time, without which the "definition changed" state has nothing to compare against, and sending a metric as a link in one action, without which the card never reaches anybody. **One was cut as an orphan:** the full lineage tree, which had a priority label and no parent.

**Build order, and 07 reads this.** The first test of the riskiest assumption needs four capabilities only: the card at a permanent URL, the named states, the visible definition with source on request, and sending it as a link. Everything else is the product that follows if the risk clears.
