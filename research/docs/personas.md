...
**Added by CJM step 4, and it raises the risk on this line rather than relieving it.** The inference above assumes the link is opened. The first outside evidence bearing on that assumption points the other way: in a comparative trial run inside a healthcare organisation, push beat pull, and a dashboard that "was linked directly within the EHR, didn't require a login, and was contextualized for the provider (basically as low friction as you could get" was visited "1-2 times per year" (`news.ycombinator.com/item?id=28330744`, dated 2021-08-27, screen `research/screens/cjm-hn-28330744-push-vs-pull.png`). Corrected during the CJM step 6 critique, because the earlier wording called that "the shape of what we are designing" and claimed more than the source carries. **What matches is the delivery conditions**, no login, placed inside the tool the person already works in, contextualised to them. **What does not match is the object:** theirs is a dashboard to explore, ours is one number with its provenance. So it measures how often a placed, frictionless destination is entered, which is the assumption underneath our design rather than our design itself, and it is measured on clinicians rather than on B2B business readers. It also reports rare entry, one to two times a year, not zero, and the two are not the same claim.
...
**Added by CJM step 4:** that provenance is what a non-technical reader is actually asking for now has a source outside vendor marketing. A practitioner reporting on interviews with clinicians: "when clinicians ask for more data, they're actually asking for more lineage or provenance metadata", and "If your dashboard can't attribute all of its data and all of the patients referenced to match the physician's definition of "theirs," you've lost" (`news.ycombinator.com/item?id=28331531`, dated 2021-08-27, screen `research/screens/cjm-hn-28331531-provenance-attribution.png`). Adjacent population, so it supports the shape of the pain rather than proving it for our reader.
